  ...
  <div id="ReviewUpdate">
    ...
    <template #overlay>
      <b-container class="mt-4 border border-light pl-3 py-4 pr-5 text-white" id="CreateBoard">
        ...
        <b-form @submit.prevent="UpdateReview()">
  ...
  methods: {
    UpdateReview() {
      ...
      if(inputReview.title === '') {
        ...
        axios({
          method: "PUT",
          url: "http://127.0.0.1:8000/movies/review/detail/" + inputReview.id + "/",
          headers: {
          ...
          data: {
            ...
            'movie': inputReview.movie,
            'id': inputReview.id,
            'user': inputReview.user.user_id,
            'like_users': inputReview.like_users
          }
        })
        .then(() => {
